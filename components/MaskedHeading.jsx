...
const MaskedHeading = ({
  ...
  const mediaSrc = src.startsWith('../public/') ? `/${src.slice('../public/'.length)}` : src;
  ...
  const sync = useCallback(() => {
    ...
    for (let i = 0; i < wordRefs.current.length; i += 1) {
      ...
      const baseline = baseRefs.current[i];
      const glyph = glyphRefs.current[i];
      if (!word || !baseline || !glyph) continue;
      ...
      <span ref={revealRef} className="masked-heading__reveal">
        <span className="masked-heading__clip" style={{ clipPath: `url(#${clipId})` }}>
          <span ref={mediaRef} className="masked-heading__media">
            {mediaType === 'video' ? (
              <video className="masked-heading__source" src={mediaSrc} poster={poster} autoPlay muted loop playsInline />
            ) : (
              <img className="masked-heading__source" src={mediaSrc} alt="" draggable={false} />
            )}
          </span>
        </span>
      </span>
